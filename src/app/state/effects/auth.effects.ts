...
export class AuthEffects {
  ...
  logins$ = createEffect(
    () => this.actions$.pipe(
      ...
      exhaustMap((action) => this.authService.login(action).pipe(
        ...
        catchError((error) => of(AuthActions.loginError({ errorMessage: error.status }))),
      ),),
    ),
  ...
  signup$ = createEffect(
    () => this.actions$.pipe(
      ...
      exhaustMap((action) => this.authService.signup(action.user.email).pipe(
        map((response) => AuthActions.signupSuccess({ user: response.user }),),
        catchError((error) => of(
  ...
  resetPassword$ = createEffect(
    () => this.actions$.pipe(
      ...
      exhaustMap((action) => this.authService.resetPassword(action.email).pipe(
  ...
  updateUser$ = createEffect(() => this.actions$.pipe(
    ...
    exhaustMap((action) => this.authService
